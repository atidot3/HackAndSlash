...
void MyUdpSocketSender::KickPlayer(int32 charID)
{
	CNtlPacket packet(sizeof(sUGR_GROUPE_KICK));
	sUGR_GROUPE_KICK * req = (sUGR_GROUPE_KICK *)packet.GetPacketData();

	req->friendAccountID = charID;
	UE_LOG(LogInit, Log, TEXT("charid = %d and friend account id = %d"), charID, req->friendAccountID);
	req->wOpCode = UGR_GROUPE_KICK;

	packet.SetPacketLen(sizeof(sUGR_GROUPE_KICK));
	SendClient(&packet);
}
