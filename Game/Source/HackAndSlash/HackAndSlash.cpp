...
void HackAndSlash::KickPlayer(int charID)
{
	senderSock->KickPlayer(charID);
}
void HackAndSlash::SendWorldMessage(FString message)
{
	std::string cstr(TCHAR_TO_UTF8(*message));
	senderSock->SendWorldMessage(cstr.c_str(), AccountID);
}