...
public:
	...
	unsigned long							GetRecvPacketCount() { return m_dwRecvPacketCount; }

	unsigned long							GetSendPacketCount() { return m_dwSendPacketCount; }
...
public:
	...
	virtual bool				Create(HWND hWnd, NtlClientNetEventCallback EventCallback, NtlClientNetLogCallback LogCallback, unsigned long dwAliveTime, int nMaxConnection) = 0;
...
public:
	...
	unsigned long						GetLocalAddr(HSERVER);
...
public:
	...
	bool						Create(HWND hWnd, NtlClientNetEventCallback EventCallback, NtlClientNetLogCallback LogCallback, unsigned long dwAliveTime, int nMaxConnection);
...
//		Return	:
//-----------------------------------------------------------------------------------
bool CNtlClientNet::Create(HWND hWnd /* = NULL */, NtlClientNetEventCallback EventCallback /* = NULL */, NtlClientNetLogCallback LogCallback /* = NULL */, unsigned long dwAliveTime /* = ALIVE_PACKET_FREQUENCY */, int nMaxConnection /* = WSA_MAXIMUM_WAIT_EVENTS */)
{
	if( NULL != m_pImp )
	{
		return false;
...
//		Return	:
//-----------------------------------------------------------------------------------
unsigned long CNtlClientNet::GetLocalAddr(HSERVER hServer)
{
...
int	CServerConnection::RecvPendingData()
{
	...
	do 
	{
		...
		if( NTL_SUCCESS != rc )
		{
			...
			printf("MakeSureCompletedPacket Fail : (%d)%s", rc, NtlGetErrorMessage(rc));
			return -1;
		}
		...
		if( false == packet.IsValidPacket() )
		{
			printf("Packet is Not Valid UsedSize[%u] DataSize[%u]", packet.GetUsedSize(), packet.GetPacketDataSize());
			return -1;
		}
		...
		if( NULL == pPacket )
		{
			printf("Packet Memory Allocate Error");
			return -1;
		}
		...
		if( m_pPacketEncoder )
		{
			...
			if( NTL_SUCCESS != rc )
			{
				printf("RxDecrypt Fail : (%d)%s", rc, NtlGetErrorMessage(rc));
				SAFE_DELETE( pPacket );
				return -1;
			}
		}
		...
		{
			printf("Packet Sequence Error Packet Sequence[%u] != Cur Sequence[%u]", pPacket->GetPacketHeader()->bySequence, m_dwRecvPacketCount & PACKET_MAX_SEQUENCE);
			SAFE_DELETE( pPacket );
			return -1;
		}
...
int CServerConnection::SendPendingData()
{
	...
	while( pPacket )	
	{
		...
		{
			std::cout << "if( m_sendBuffer.GetFreeSize() < pPacket->GetUsedSize() )" << std::endl;
			break;
		}
		...
		if( m_pPacketEncoder )
		{
			...
			if( NTL_SUCCESS != rc )
			{
				printf("TxDecrypt Fail : (%d)%s", rc, NtlGetErrorMessage(rc));
				std::cout << "TxDecrypt Fail" << std::endl;
				return -1;
			}
		}
#endif
...
void CClientNetImp::Destroy()
{
	...
	if( INVALID_HANDLE_VALUE != m_hThread )
	{
		...
		if( 0xFFFFFFFF == rc )
		{
			printf("WaitForSingleObject Fail : (%d)%s", rc, NtlGetErrorMessage(rc));
		}
		else
		{
...
//		Return	:
//-----------------------------------------------------------------------------------
unsigned long CClientNetImp::GetLocalAddr(HSERVER hServer)
{
	CAutoPtr_ServerConnection pConnection = Acquire( hServer );
	if( false == pConnection.IsValidPtr() )
	{
...
//		Return	:
//-----------------------------------------------------------------------------------
bool CMsgBaseNetworkImp::Create(HWND hWnd, NtlClientNetEventCallback EventCallback, NtlClientNetLogCallback LogCallback, unsigned long dwAliveTime, int nMaxConnection)
{
...
bool CMsgBaseNetworkImp::Connect(char* szServerAddress, WORD wPort, HSERVER * pServerHandle, bool bAsync, eNETCLIENT_ENCODE_TYPE encode, int nPacketCount, int nPacketSize)
{
	...
	if( NTL_SUCCESS != rc )
	{
		printf("Socket Create Error : (%d)%s", rc, NtlGetErrorMessage(rc));
		return false;
	}
	...
	if( bAsync )
	{
		...
		if( NTL_SUCCESS != rc )
		{
			printf("Socket Set NonBlocking Error : (%d)%s", rc, NtlGetErrorMessage(rc));
			return false;
		}
	}
	...
	if( NULL == pConnection )
	{
		printf("Connection Memory Allocate Error");
		return false;
	}
	...
	{
		printf("Connection Create Error remoteAddr [%s]", remoteAddr.GetDottedAddr());
	...
	else
	{
		...
		{
			printf("Socket Connect Error : (%d)%s", rc, NtlGetErrorMessage(rc));
			RELEASE_CONNECTION( pConnection );
			return false;
		}
	}
	...
	{
		printf("Connection List Insert Error");
		RELEASE_CONNECTION( pConnection );
		return false;
	}
	mutex.Unlock();
	...
	*pServerHandle = pConnection->GetRawSocket();
	...
	{
		rc = WSAGetLastError();
		printf( "Socket AsyncSelect Error : (%d)%s", rc, NtlGetErrorMessage( rc ) );
		return false;
	}
...
bool CMsgBaseNetworkImp::Disconnect(HSERVER hServer)
{
	...
	if( ERROR_SUCCESS != rc )
	{
		printf( "Socket shutdown Error : (%d)%s", rc, NtlGetErrorMessage( rc ) );
		return false;
	}
...
bool CMsgBaseNetworkImp::NetMsgProc(WPARAM wParam, LPARAM lParam)
{
	...
	{
		//정상적으로 찾지 못할 경우가 있다.
		//printf( "Socket NetMsgProc Error, Find Error Connection[%u]", wParam );
		return false;
	}
	...
	case FD_READ :
		{
			unsigned long dwRecvBytes = pConnection->Recv();
			if( SOCKET_ERROR == dwRecvBytes )
			{
				int rc = WSAGetLastError();
				printf( "Connection Recv Error (%d)%s", rc, NtlGetErrorMessage( rc ) );
	...
	case FD_WRITE :
		{
			if( pConnection->SendPendingData() > 0 )
			{
				if( SOCKET_ERROR == pConnection->Send() )
				{
					...
					if( WSAEWOULDBLOCK != rc )
					{
						printf( "Connection Send Error (%d)%s", rc, NtlGetErrorMessage( rc ) );
						InternalDisconnect( pConnection->GetHandle() );
					}
				}
			}
...
bool CMsgBaseNetworkImp::PushPacket(HSERVER hServer, void * pData, int nDataSize)
{
	...
	{
		printf( "PushPacket Error, Packet wrong" );
		SAFE_DELETE( pPacket );
		return false;
	}
...
//		Return	:
//-----------------------------------------------------------------------------------
bool CEventBaseNetworkImp::Create(HWND hWnd, NtlClientNetEventCallback EventCallback, NtlClientNetLogCallback LogCallback, unsigned long dwAliveTime, int nMaxConnection)
{
...
#endif
{
	...
	{
		printf( "There is no more Event Objects" );
		return false;
	}
	...
	if( false == pConnection->Create( this, socket, remoteAddr, encode, nPacketCount, nPacketSize ) )
	{
		printf( "Connection Create Error" );
		RELEASE_CONNECTION( pConnection );
		return false;
	}
...
void CEventBaseNetworkImp::OnThreadProc()
{
	...
	unsigned long dwTickOld = 0;
	unsigned long dwTickCur = 0;
	...
	while( m_bProgress )
	{
		...
		unsigned long dwEventTotal = (unsigned long)m_nCurConnection;
		...
		{
			int rc = WSAGetLastError();
			printf( "WSAWaitForMultipleEvents Fail (%d)%s", rc, NtlGetErrorMessage(rc) );
			continue;
		}
		...
		//for( unsigned long i = dwIndex; i < dwEventTotal; i++ )
		//{
		...
		{
			int rc = WSAGetLastError();
			printf( "WSAEnumNetworkEvents Fail (%d)%s", rc, NtlGetErrorMessage(rc) );
			continue;
		}
		...
		if( wsaNetworkEvents.lNetworkEvents & FD_CONNECT )
		{
			...
			{					
				...
				printf( "WSAEnumNetworkEvents FD_CONNECT_BIT Fail (%d)%s", rc, NtlGetErrorMessage(rc) );
		...
		if( wsaNetworkEvents.lNetworkEvents & FD_READ )
		{
			if( 0 != wsaNetworkEvents.iErrorCode[FD_READ_BIT] )
			{
				...
				printf( "WSAEnumNetworkEvents FD_READ Fail (%d)%s", rc, NtlGetErrorMessage(rc) );
				continue;						
			}
			...
			else if( dwRecvBytes > 0 )
			{
				...
				if( nResult < 0 )
				{
					printf( "Connection RecvPendingData Error, Result(%d)", nResult );
					InternalDisconnect( pConnection->GetHandle() );
				}
			}
			else
		...
		if( wsaNetworkEvents.lNetworkEvents & FD_WRITE )
		{
			if( 0 != wsaNetworkEvents.iErrorCode[FD_WRITE_BIT] )
			{
				...
				printf( "WSAEnumNetworkEvents FD_WRITE Fail (%d)%s", rc, NtlGetErrorMessage(rc) );
				continue;						
			}
		...
		if( wsaNetworkEvents.lNetworkEvents & FD_CLOSE )
		{
			if( 0 != wsaNetworkEvents.iErrorCode[FD_CLOSE_BIT] )
			{
				...
				printf( "WSAEnumNetworkEvents FD_CLOSE_BIT Fail (%d)%s", rc, NtlGetErrorMessage(rc) );
			}
