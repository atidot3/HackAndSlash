...
const unsigned long				RX_RANDOM_SEED_HEADER = 0xFFEECCBB;

const unsigned long				TX_RANDOM_SEED_HEADER = 0xBBCCEEFF;

const unsigned long				RX_RANDOM_SEED_BODY = 0xAABBCCDD;

const unsigned long				TX_RANDOM_SEED_BODY = 0xDDBBCCAA;
...
//-----------------------------------------------------------------------------------
int Encrypt(BYTE * pPlain, int nLength, unsigned long dwCipherKey, BYTE * pbyCheckSum = NULL)
{
	if ( NULL == pPlain )
	{
		return 0;
	...
	int nRound = nLength / sizeof(unsigned long);
	unsigned long * pdwRoundPlain = (unsigned long*)pPlain;
	for( int i = 0; i < nRound; i++ )
	{
		...
		pdwRoundPlain[i] ^= (unsigned long)dwCipherKey;
	}


	int nRest = nLength % sizeof(unsigned long);
...
//		Return	:
//-----------------------------------------------------------------------------------
int Decrypt(BYTE * pPlain, int nLength, unsigned long dwCipherKey, BYTE * pbyCheckSum = NULL)
{
	if ( NULL == pPlain )
	{
		return 0;
...
int CNtlPacketEncoder_RandKey::RxDecrypt(CNtlPacket& rPacket)
{
	...
	// Checksum
	printf("Rx PacketLen[%u] Sequence[%u] Checksum[%u]", rPacket.GetPacketHeader()->wPacketLen, rPacket.GetPacketHeader()->bySequence, rPacket.GetPacketHeader()->byChecksum );
...
int CNtlPacketEncoder_RandKey::TxEncrypt(CNtlPacket& rPacket)
{
	...
	printf("TxEncryptPacketLen[%u] Sequence[%u] Checksum[%u]", rPacket.GetPacketHeader()->wPacketLen, rPacket.GetPacketHeader()->bySequence, rPacket.GetPacketHeader()->byChecksum);
